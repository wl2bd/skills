...
2. **Fails the radio test**: say it aloud once to someone who has never seen it; if they can't spell it, every referral, podcast mention, and word-of-mouth moment will leak. (Also called the phone test / bar test.) For a name that must carry a separator (a package, a CLI, a skill folder), the test includes it: heard once, does the listener type `naming-things`, `namingthings` or `naming_things`?
